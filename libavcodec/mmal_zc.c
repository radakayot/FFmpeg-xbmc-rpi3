...
typedef struct AVZcEnv
{
    ...
    void * pool_env;
    av_mmal_zc_alloc_buf_fn_t * alloc_buf;
    av_mmal_zc_free_pool_fn_t * free_pool;
...
typedef struct ZcUserBufEnv {
    void * v;
    const av_mmal_zc_buf_fn_tab_t * fn;
    size_t numbytes;
    int offset;
} ZcUserBufEnv;
...
/*
static inline int av_mmal_is_sand_format(const int format)
{
    return (format >= AV_PIX_FMT_SAND128 && format <= AV_PIX_FMT_SAND64_16);
}

static inline int av_mmal_is_sand_frame(const AVFrame * const frame)
{
    return av_mmal_is_sand_format(frame->format);
}
*/
//----------------------------------------------------------------------------
//
...
static ZcPoolEnt * zc_pool_ent_alloc(ZcPool * const pool, const size_t req_size)
{
    ...
    if ((zp->vcsm_handle = vcsm_malloc_cache(alloc_size, VCSM_CACHE_TYPE_HOST | 0x80, "ffmpeg_mmal_zc")) == 0)
    {
...
static const av_mmal_zc_buf_fn_tab_t zc_pool_buf_fns = {
    .free        = zc_pool_free_v,
...
static AVBufferRef *
zc_pool_buf_alloc(void * v, size_t size, const AVMMALZcFrameGeometry * geo)
{
    ZcPool * const pool = v;
    ...
    if ((buf = av_mmal_zc_buf(size, 0, zp, &zc_pool_buf_fns)) == NULL)
    {
        av_log(NULL, AV_LOG_ERROR, "av_mmal_zc_buf() failed\n");
        goto fail2;
    }
...
AVZcEnvPtr
av_mmal_zc_int_env_alloc(void * logctx)
{
    ZcEnv * zc;
    ZcPool * pool_env;

    //if (mmal_mem_gpu_init(0) < 0)
    //    return NULL;

    if ((pool_env = zc_pool_new()) == NULL)
        goto fail1;

    if ((zc = av_mmal_zc_env_alloc(logctx, pool_env, zc_pool_buf_alloc, zc_pool_delete_v)) == NULL)
        goto fail2;
...
void
av_mmal_zc_int_env_freep(AVZcEnvPtr * zcp)
{
    const AVZcEnvPtr zc = *zcp;
    *zcp = NULL;
    if (zc != NULL)
        av_mmal_zc_env_release(zc);
}
...
AVMMALZcFrameGeometry av_mmal_zc_frame_geometry(
    ...
    const AVFrame * const src)
{
    ...
    if (av_mmal_zc_get_buffer(zc, dest) != 0 ||
        av_mmal_zc_resolve_frame(dest, ZC_RESOLVE_ALLOC_VALID) != 0)
    {
        return NULL;
    }
    ...
    const AVFrame * const src, const unsigned int src_bits)
{
    ...
    // Y
    av_mmal_sand16_to_sand8(dest->data[0], dest->linesize[0], av_mmal_sand_frame_stride2(dest),
                        src->data[0], src->linesize[0], av_mmal_sand_frame_stride2(dest),
                        src->width, src->height, shr);
    // C
    av_mmal_sand16_to_sand8(dest->data[1], dest->linesize[1], av_mmal_sand_frame_stride2(dest),
                        src->data[1], src->linesize[1], av_mmal_sand_frame_stride2(dest),
...
unsigned int av_mmal_zc_vcsm_handle(const AVMMALZcRefPtr fr_ref)
{
    const GPU_MEM_PTR_T * const p = pic_gm_ptr(fr_ref);
    return p == NULL ? 0 : p->vcsm_handle;
}

int av_mmal_zc_vc_handle(const AVMMALZcRefPtr fr_ref)
{
    const GPU_MEM_PTR_T * const p = pic_gm_ptr(fr_ref);
    return p == NULL ? -1 : p->vc_handle;
}

int av_mmal_zc_offset(const AVMMALZcRefPtr fr_ref)
{
    const ZcBufEnv * const zbe = pic_zbe_ptr(fr_ref);
    return zbe == NULL ? 0 : zbe->offset;
}

int av_mmal_zc_length(const AVMMALZcRefPtr fr_ref)
{
    const ZcBufEnv * const zbe = pic_zbe_ptr(fr_ref);
    return zbe == NULL ? 0 : zbe->size_pic;
}

int av_mmal_zc_numbytes(const AVMMALZcRefPtr fr_ref)
{
    const GPU_MEM_PTR_T * const p = pic_gm_ptr(fr_ref);
    return p == NULL ? 0 : p->numbytes;
}

const AVMMALZcFrameGeometry * av_mmal_zc_geometry(const AVMMALZcRefPtr fr_ref)
{
    const ZcBufEnv * const zbe = pic_zbe_ptr(fr_ref);
    return zbe == NULL ? NULL : &zbe->geo;
}

AVMMALZcRefPtr av_mmal_zc_ref(const AVZcEnvPtr zc,
    const AVFrame * const frame, const enum AVPixelFormat expected_format, const int maycopy)
{
    ...
    if (frame->format != AV_PIX_FMT_YUV420P &&
        ...
        !av_mmal_is_sand_frame(frame))
    {
        av_log(NULL, AV_LOG_WARNING, "%s: *** Format not SAND/YUV420P: %d\n", __func__, frame->format);
        return NULL;
    }
    ...
    if (frame->buf[1] != NULL || frame->format != expected_format)
    {
        ...
        if (frame->format == AV_PIX_FMT_SAND64_10 && expected_format == AV_PIX_FMT_SAND128 && frame->buf[MMAL_ZC_SAND_8_IN_10_BUF] != NULL)
        {
//            av_log(s, AV_LOG_INFO, "%s: --- found buf[4]\n", __func__);
            return av_buffer_ref(frame->buf[MMAL_ZC_SAND_8_IN_10_BUF]);
        }
#endif
        ...
        {
            if (frame->buf[1] != NULL)
                av_log(NULL, AV_LOG_INFO, "%s: *** Not a single buf frame: copying\n", __func__);
            else
                av_log(NULL, AV_LOG_INFO, "%s: *** Unexpected frame format %d: copying to %d\n", __func__, frame->format, expected_format);
        ...
        {
            if (frame->buf[1] != NULL)
                av_log(NULL, AV_LOG_WARNING, "%s: *** Not a single buf frame: buf[1] != NULL\n", __func__);
            else
                av_log(NULL, AV_LOG_INFO, "%s: *** Unexpected frame format: %d != %d\n", __func__, frame->format, expected_format);
            return NULL;
        }
    }
    ...
    if (pic_gm_ptr(frame->buf[0]) == NULL)
    {
        if (maycopy)
        {
            av_log(NULL, AV_LOG_INFO, "%s: *** Not one of our buffers: copying\n", __func__);
            return zc_copy(zc, frame);
        }
        else
        {
            av_log(NULL, AV_LOG_WARNING, "%s: *** Not one of our buffers: NULL\n", __func__);
            return NULL;
        }
    }
...
void av_mmal_zc_unref(AVMMALZcRefPtr fr_ref)
{
    if (fr_ref != NULL)
    {
...
void * av_mmal_zc_buf_v(AVBufferRef * const buf)
{
...
AVBufferRef * av_mmal_zc_buf(size_t numbytes, int addr_offset, void * v, const av_mmal_zc_buf_fn_tab_t * fn_tab)
{
    AVBufferRef *buf;
    ZcUserBufEnv * zub;
...
int av_mmal_zc_resolve_buffer(AVBufferRef * const buf, const int alloc_mode)
{
    ZcBufEnv * const zbe = pic_zbe_ptr(buf);
    ...
    if (zbe->user == NULL)
    {
        ...
        {
            av_log(NULL, AV_LOG_ERROR, "mmal_get_display_buffer: Failed to get buffer from pool\n");
            goto fail;
        }
...
int av_mmal_zc_resolve_frame(AVFrame * const frame, const int may_alloc)
{
    int rv;

    // Do alloc if we need it
    if ((rv = av_mmal_zc_resolve_buffer(frame->buf[0], may_alloc)) != 0)
        return rv;
...
int av_mmal_zc_set_valid_frame(AVFrame * const frame)
{
...
int av_mmal_zc_set_broken_frame(AVFrame * const frame)
{
    ZcBufEnv * const zbe = pic_zbe_ptr(frame->buf[0]);
...
void av_mmal_zc_set_decoder_pool_size(ZcEnv *const zc, const unsigned int pool_size)
{
    zc->pool_size = pool_size;
}

unsigned int av_mmal_zc_get_decoder_pool_size(ZcEnv *const zc)
{
    return zc->pool_size;
}

int av_mmal_zc_get_buffer(ZcEnv *const zc, AVFrame * const frame)
{
#if 1
    ...
    zbe->geo = av_mmal_zc_frame_geometry(frame->format, frame->width, frame->height);  // Note geometry for later use
...
#else
    const AVMMALZcFrameGeometry geo = av_mmal_zc_frame_geometry(frame->format, frame->width, frame->height);
...
#if MMAL_ZC_SAND_8_IN_10_BUF != 0
    // *** If we intend to use this for real we will want a 2nd buffer pool
    frame->buf[MMAL_ZC_SAND_8_IN_10_BUF] = zc_pool_buf_alloc(&zc->pool, size_pic);  // *** 2 * wanted size - kludge
#endif
#endif
...
void av_mmal_zc_env_release(const AVZcEnvPtr zc)
{
...
AVZcEnvPtr av_mmal_zc_env_alloc(void * logctx,
                    void * pool_env,
                    av_mmal_zc_alloc_buf_fn_t * alloc_buf_fn,
                    av_mmal_zc_free_pool_fn_t * free_pool_fn)
{
    ZcEnv * zc;

    if ((zc = av_mallocz(sizeof(ZcEnv))) == NULL)
    {
        av_log(logctx, AV_LOG_ERROR, "av_mmal_zc_env_alloc: Context allocation failed\n");
        return NULL;
    }
...
//        printf("Do default alloc: format=%#x\n", frame->format);
        rv = avcodec_default_get_buffer2(s, frame, flags);
             ...
             av_mmal_is_sand_frame(frame))
    {
        if ((rv = av_mmal_zc_get_buffer(s->opaque, frame)) == 0)
            rv = av_mmal_zc_resolve_frame(frame, ZC_RESOLVE_ALLOC_VALID);
    }
    else
    {
        rv = avcodec_default_get_buffer2(s, frame, flags);
...
int av_mmal_zc_in_use(const struct AVCodecContext * const s)
{
    return s->get_buffer2 == zc_get_buffer2;
}

int av_mmal_zc_init2(struct AVCodecContext * const s,
                    void * pool_env,
                    av_mmal_zc_alloc_buf_fn_t * alloc_buf_fn,
                    av_mmal_zc_free_pool_fn_t * free_pool_fn)
{
    ...
    av_assert0(!av_mmal_zc_in_use(s));

    if ((zc = av_mmal_zc_env_alloc(s, pool_env, alloc_buf_fn, free_pool_fn)) == NULL)
        return AVERROR(ENOMEM);
...
void av_mmal_zc_uninit2(struct AVCodecContext * const s)
{
