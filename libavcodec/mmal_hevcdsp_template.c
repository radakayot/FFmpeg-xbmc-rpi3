...
//    mmal_sand_dump16("ARC In Pred", _dst, stride, 0, 0, 0, size, size, 1);
//    mmal_sand_dump16("ARC In RU", ru, size * 2, 0, 0, 0, size, size, 0);
//    mmal_sand_dump16("ARC In RV", rv, size * 2, 0, 0, 0, size, size, 0);
...
//    mmal_sand_dump16("ARC Out", _dst, stride * 2, 0, 0, 0, size, size, 1);
}
...
static void FUNC(sao_edge_filter)(uint8_t *_dst, uint8_t *_src, ptrdiff_t stride_dst, int16_t *sao_offset_val,
    ...
    int x, y;
    const ptrdiff_t stride_src = MMAL_HEVC_SAO_BUF_STRIDE / sizeof(pixel);
    stride_dst /= sizeof(pixel);
...
static void FUNC(sao_edge_restore_0)(uint8_t *_dst, uint8_t *_src,
                                    ptrdiff_t stride_dst, ptrdiff_t stride_src, MMALSAOParams *sao,
                                  ...
                                  int height, intptr_t mx, intptr_t my, int width)
{
    ...
    pixel        *src       = (pixel*)_src;
    ptrdiff_t     srcstride = _srcstride / sizeof(pixel);
    const int8_t *filter    = ff_hevc_mmal_qpel_filters[mx - 1];
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++)
                                  ...
                                  int height, intptr_t mx, intptr_t my, int width)
{
    ...
    pixel        *src       = (pixel*)_src;
    ptrdiff_t     srcstride = _srcstride / sizeof(pixel);
    const int8_t *filter    = ff_hevc_mmal_qpel_filters[my - 1];
    for (y = 0; y < height; y++)  {
        for (x = 0; x < width; x++)
                                   ...
                                   intptr_t my, int width)
{
    ...
    filter = ff_hevc_mmal_qpel_filters[mx - 1];
    ...
    filter = ff_hevc_mmal_qpel_filters[my - 1];
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++)
                                  ...
                                  int height, intptr_t mx, intptr_t my, int width)
{
    ...
    pixel *src = (pixel *)_src;
    ptrdiff_t srcstride  = _srcstride / sizeof(pixel);
    const int8_t *filter = ff_hevc_mmal_epel_filters[mx - 1];
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++)
                                  ...
                                  int height, intptr_t mx, intptr_t my, int width)
{
    ...
    pixel *src = (pixel *)_src;
    ptrdiff_t srcstride = _srcstride / sizeof(pixel);
    const int8_t *filter = ff_hevc_mmal_epel_filters[my - 1];
                                   ...
                                   int height, intptr_t mx, intptr_t my, int width)
{
    ...
    filter = ff_hevc_mmal_epel_filters[my - 1];
