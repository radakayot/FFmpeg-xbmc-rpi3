...
static av_always_inline int compare_mv_ref_idx(const HEVCMMALMvField * const a, const HEVCMMALMvField * const b)
{
 ...
 */
static int temporal_luma_motion_vector(const HEVCMMALContext * const s,
                                       const HEVCMMALLocalContext * const lc, const int x0, const int y0,
                                       ...
                                       MvXY * const mvLXCol, const int X)
{
    ...
    HEVCMMALFrame * const col_ref = s->ref->collocated_ref;
    ...
    *mvLXCol = 0;
    ...
    ff_hevc_mmal_progress_wait_mv(s, lc->jb0, col_ref, y0 + nPbH);
...
static inline int mvf_eq(const HEVCMMALMvField * const a, const HEVCMMALMvField * const b)
{
 ...
 */
static inline const HEVCMMALMvField *
derive_spatial_merge_candidates(
    const HEVCMMALContext * const s,
    ...
    HEVCMMALMvField * const mvf_t)
{
    ...
    const HEVCMMALMvField * mvf_a1 = mvf_ptr(s, lc, x0, y0, x0 - 1, y0 + nPbH - 1);
    const HEVCMMALMvField * mvf_a0 = mvf_a1 + mvf_left_stride(s, x0, x0 - 1);
    const HEVCMMALMvField * mvf_b1 = mvf_ptr(s, lc, x0, y0, x0 + nPbW - 1, y0 - 1);
    const HEVCMMALMvField * mvf_b0 = mvf_b1 + 1;
    const unsigned int plevel = s->ps.pps->log2_parallel_merge_level;
    const unsigned int part_mode = lc->cu.part_mode;

    const HEVCMMALMvField * perm[4];
    ...
    {
        const HEVCMMALMvField * mvf_b2 = mvf_ptr(s, lc, x0, y0, x0 - 1, y0 - 1);  // UL
    ...
    {
        static const HEVCMMALMvField mvf_z = {{0}};
    ...
    if (s->sh.slice_type == HEVC_SLICE_B && nb_merge_cand > 1)
    {
        ...
        for (comb_idx = 0; comb_idx < cand_count; comb_idx++)
        {
            ...
            const HEVCMMALMvField * const mvf_c0 = perm[l0_cand_idx];
            const HEVCMMALMvField * const mvf_c1 = perm[l1_cand_idx];
...
{
    const HEVCMMALMvField * mvf_m = (s->ps.pps->log2_parallel_merge_level > 2 && log2_cb_size == 3) ?
        derive_spatial_merge_candidates(s, lc, lc->cu.x, lc->cu.y, 8, 8,
                                        ff_hevc_mmal_tb_avail_flags(s, lc, lc->cu.x, lc->cu.y, 8, 8),
                                        0, merge_idx, mv) :
        derive_spatial_merge_candidates(s, lc, x0, y0, nPbW, nPbH,
                                        ff_hevc_mmal_tb_avail_flags(s, lc, x0, y0, nPbW, nPbH),
...
mvf_same_poc(const RefPicList * const rpl, const unsigned int pfi0, const unsigned int pfi1, const int poc0, const HEVCMMALMvField * const mvf)
{
    if (mvf != NULL)
    {
...
mvf_other_poc(const RefPicList * const rpl, const unsigned int pfi0, const unsigned int pfi1,
              ...
              MvXY * const mv_t, const HEVCMMALMvField * const mvf)
{
    if (mvf != NULL)
    {
