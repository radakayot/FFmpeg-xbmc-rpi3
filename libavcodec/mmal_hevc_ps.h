...
#ifndef AVCODEC_MMAL_HEVC_PS_H
#define AVCODEC_MMAL_HEVC_PS_H
...
typedef struct MMALSliceHeader {
    unsigned int pps_id;
...
typedef struct HEVCMMALSPS {
    unsigned vps_id;
    uint8_t chroma_format_idc;
    uint8_t separate_colour_plane_flag;

    HEVCMMALWindow output_window;

    HEVCMMALWindow pic_conf_win;
...
typedef struct HEVCMMALParamSets {
    /* currently active parameter sets */
    const HEVCMMALVPS *vps;
    const HEVCMMALSPS *sps;
    const HEVCMMALPPS *pps;
...
int ff_hevc_mmal_decode_nal_vps(GetBitContext *gb, AVCodecContext *avctx,
...
int ff_hevc_mmal_decode_nal_pps(GetBitContext *gb, AVCodecContext *avctx,
                           HEVCMMALParamSets *ps);

int ff_hevc_mmal_decode_short_term_rps(GetBitContext *gb, AVCodecContext *avctx,
                                  ShortTermRPS *rps, const HEVCMMALSPS *sps, int is_slice_header);

int ff_hevc_mmal_encode_nal_vps(HEVCMMALVPS *vps, unsigned int id,
