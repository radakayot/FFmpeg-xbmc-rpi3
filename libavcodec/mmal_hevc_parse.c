...
static int hevc_decode_nal_units(const uint8_t *buf, int buf_size, HEVCMMALParamSets *ps,
                                 ...
                                 int err_recognition, int apply_defdispwin, void *logctx)
{
    ...
    for (i = 0; i < pkt.nb_nals; i++) {
        ...
        switch (nal->type) {
        case HEVC_NAL_VPS:
            ret = ff_hevc_mmal_decode_nal_vps(&nal->gb, logctx, ps);
            if (ret < 0)
                goto done;
            break;
        case HEVC_NAL_SPS:
            ret = ff_hevc_mmal_decode_nal_sps(&nal->gb, logctx, ps, apply_defdispwin);
            if (ret < 0)
                goto done;
            break;
        case HEVC_NAL_PPS:
            ret = ff_hevc_mmal_decode_nal_pps(&nal->gb, logctx, ps);
            if (ret < 0)
                goto done;
            break;
        case HEVC_NAL_SEI_PREFIX:
        case HEVC_NAL_SEI_SUFFIX:
            ret = ff_hevc_mmal_decode_nal_sei(&nal->gb, logctx, sei, ps, nal->type);
            if (ret < 0)
                goto done;
            break;
        default:
