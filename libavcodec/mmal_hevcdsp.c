...
DECLARE_ALIGNED(16, const int8_t, ff_hevc_mmal_epel_filters[7][4]) = {
    { -2, 58, 10, -2},
    { -4, 54, 16, -2},
    { -6, 46, 28, -4},
    { -4, 36, 36, -4},
...
DECLARE_ALIGNED(16, const int8_t, ff_hevc_mmal_qpel_filters[3][16]) = {
...
#define BIT_DEPTH 8
#include "mmal_hevcdsp_template.c"
#undef BIT_DEPTH
...
static uint32_t hevc_deblocking_boundary_strengths(int pus, int dup, const HEVCMMALMvField *curr, const HEVCMMALMvField *neigh,
                                               ...
                                               int in_inc0, int in_inc1)
{
    ...
    for (; pus > 0; pus--) {
        ...
        curr += in_inc0 / sizeof (HEVCMMALMvField);
        neigh += in_inc1 / sizeof (HEVCMMALMvField);
...
void ff_hevc_mmal_dsp_init(HEVCDSPContext *hevcdsp, int bit_depth)
{
#undef FUNC
...
int i = 0;
    ...
    if (ARCH_PPC)
        ff_hevc_mmal_dsp_init_ppc(hevcdsp, bit_depth);
    if (ARCH_X86)
        ff_hevc_mmal_dsp_init_x86(hevcdsp, bit_depth);
    if (ARCH_ARM)
        ff_hevcdsp_mmal_init_arm(hevcdsp, bit_depth);
    if (ARCH_MIPS)
        ff_hevc_mmal_dsp_init_mips(hevcdsp, bit_depth);
}
