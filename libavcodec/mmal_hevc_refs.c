...
void ff_hevc_mmal_clear_refs(HEVCMMALContext *s)
{
    ...
    for (i = 0; i < FF_ARRAY_ELEMS(s->DPB); i++)
        ff_hevc_mmal_unref_frame(s, &s->DPB[i],
...
static HEVCMMALFrame *alloc_frame(HEVCMMALContext * const s)
{
    int i, ret;
    for (i = 0; i < FF_ARRAY_ELEMS(s->DPB); i++) {
        HEVCMMALFrame * const frame = &s->DPB[i];
...
fail:
        ff_hevc_mmal_unref_frame(s, frame, ~0);
        return NULL;
    }
...
int ff_hevc_mmal_output_frame(HEVCMMALContext *s, AVFrame *out, int flush)
{
    do {
        int nb_output = 0;
        int min_poc   = INT_MAX;
        ...
        if (s->sh.no_output_of_prior_pics_flag == 1 && s->no_rasl_output_flag == 1) {
            for (i = 0; i < FF_ARRAY_ELEMS(s->DPB); i++) {
                ...
                if (!(frame->flags & HEVC_FRAME_FLAG_BUMPING) && frame->poc != s->poc &&
                    ...
                    ff_hevc_mmal_unref_frame(s, frame, HEVC_FRAME_FLAG_OUTPUT);
                }
            }
        }
        ...
        if (nb_output) {
            HEVCMMALFrame *frame = &s->DPB[min_idx];
            if (frame->frame->format == AV_PIX_FMT_VIDEOTOOLBOX && frame->frame->buf[0]->size == 1)
                return 0;

            ret = av_frame_ref(out, frame->frame);
            if (frame->flags & HEVC_FRAME_FLAG_BUMPING)
                ff_hevc_mmal_unref_frame(s, frame, HEVC_FRAME_FLAG_OUTPUT | HEVC_FRAME_FLAG_BUMPING);
            else
                ff_hevc_mmal_unref_frame(s, frame, HEVC_FRAME_FLAG_OUTPUT);
            if (ret < 0)
                return ret;
...
static int init_slice_rpl(HEVCMMALContext *s)
{
...
static HEVCMMALFrame *find_ref_idx(HEVCMMALContext *s, int poc)
{
    int i;
    int LtMask = (1 << s->ps.sps->log2_max_poc_lsb) - 1;

    for (i = 0; i < FF_ARRAY_ELEMS(s->DPB); i++) {
        HEVCMMALFrame *ref = &s->DPB[i];
...
static void mark_ref(HEVCMMALFrame *frame, int flag)
{
    frame->flags &= ~(HEVC_FRAME_FLAG_LONG_REF | HEVC_FRAME_FLAG_SHORT_REF);
    frame->flags |= flag;
}

static HEVCMMALFrame *generate_missing_ref(HEVCMMALContext *s, int poc)
{
    HEVCMMALFrame *frame;
    int i, x, y;
    ...
    ff_hevc_mmal_progress_set_all_done(frame);

    return frame;
}

/* add a reference with the given poc to the list and mark it as used in DPB */
static int add_candidate_ref(HEVCMMALContext *s, RefPicList *list,
                             int poc, int ref_flag)
{
    HEVCMMALFrame *ref = find_ref_idx(s, poc);
