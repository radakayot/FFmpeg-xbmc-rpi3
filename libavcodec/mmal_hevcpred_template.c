...
static void FUNC(intra_pred)(const HEVCMMALContext * const s,
                                              ...
                                              const unsigned int log2_size)
{
    ...
    pixel *const src = c_idx == 0 ?
        (pixel *)av_mmal_sand_frame_pos_y(s->frame, x, y) :
        (pixel *)av_mmal_sand_frame_pos_c(s->frame, x, y);
...
dc_only:
...

    {
        ...
        const unsigned int stripe_adj = (av_mmal_sand_frame_stride2(frame) - 1) * stride;
        if ((x & mask) == 0)
            src_l -= stripe_adj;
        if (((x + size) & mask) == 0)
            src_ur += stripe_adj;
